...
console.log(`1. Fichier .env.local: ${envExists ? '✅ Existe' : '❌ Manquant'}`);
...
if (envExists) {
  try {
    ...
    console.log(`2. NEXTAUTH_SECRET: ${hasSecret ? '✅ Défini' : '❌ Manquant'}`);
    ...
    if (hasSecret) {
      ...
      if (!secret) {
        ...
        console.log(`   ⚠️  NEXTAUTH_SECRET trop court (${secret.length} caractères, minimum 32 recommandé)`);
      } else {
        console.log(`   ✅ NEXTAUTH_SECRET valide (${secret.length} caractères)`);
      }
    }
    
    // Vérifier NEXTAUTH_URL
    const hasUrl = lines.some(line => line.startsWith('NEXTAUTH_URL='));
    console.log(`3. NEXTAUTH_URL: ${hasUrl ? '✅ Défini' : '⚠️  Optionnel (utilise http://localhost:3000 par défaut)'}`);
  ...
  console.log('$secret = [Convert]::ToBase64String((1..32 | ForEach-Object { Get-Random -Maximum 256 }))');
  console.log('"NEXTAUTH_SECRET=$secret" | Out-File -FilePath .env.local -Encoding utf8');
}
